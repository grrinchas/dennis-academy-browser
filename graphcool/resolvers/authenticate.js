...
const createGraphCoolUser = (auth0UserId, email, username, picture, api) =>
    api
        .request(
            `
        mutation createUser($auth0UserId: String!, $email: String!, $username: String!, $picture: String!) {
          createUser(
      ...
      `,
            {auth0UserId, email, username, picture}
        )
        .then(queryResult => queryResult.createUser)

const fetchAuth0 = accessToken =>
    fetch(
...
export default async event => {
    try {
        ...
        if (graphCoolUser === null) {
            ...
            let user = await fetchAuth0(accessToken)

            graphCoolUser = await createGraphCoolUser(decodedToken.sub, user.email, user.nickname, user.picture, api)
        }
